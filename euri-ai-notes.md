...
documentation refer: euri ai models documentation: https://docs.euri.ai/quickstart
...
> ⭐ **Suggested method:** Store the key in a `.env` file and load it with `python-dotenv`. This keeps secrets out of your source code and out of git, and is the standard, recommended practice for any real project.

```bash
pip install python-dotenv
# or: uv add python-dotenv
```

`.env` file (place in your project root, and add `.env` to `.gitignore`):
```
...
Notes:
- `load_dotenv()` must run *before* you access `os.environ["EURI_API_KEY"]`, or you'll get a `KeyError`.
- If your script runs from a different working directory than the `.env` file, be explicit: `load_dotenv(dotenv_path="/full/path/to/.env")`.
- Safer alternative to avoid a hard crash if the key is missing: use `os.environ.get("EURI_API_KEY")` and check for `None` before creating the client.

Other equally valid but less convenient ways:
